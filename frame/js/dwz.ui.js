...
function initUI($p){
	...
	$("a[target=navTab]", $p).each(function(){
		$(this).click(function(event){
			...
			//anlb增加
			var beforeClickEvent=$this.attr("before");
			if(beforeClickEvent)
				eval(beforeClickEvent);
			
			var title = $this.attr("title") || $this.text();
			var tabid = $this.attr("rel") || "_blank";
			
			//anlb增加
			if(tabid=="currentNavTab") {tabid=navTab.getCurrentTabId();title=navTab.getTitle();};
	...
	//anlb增加(初始化checkbox,radiobox)
	$("[checkedValue]",$p).each(function(){
		var checkedValue=$(this).attr("checkedValue");
		
		$("input",$(this)).each(function(){
			if(checkedValue.indexOf($(this).val())!=-1||checkedValue==$(this).val())
