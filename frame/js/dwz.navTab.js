...
var navTab = {
	...
	//anlb增加
	switchTab: function(openTabid){
		var iTabIndex=this._indexTabId(openTabid);
		if(iTabIndex>0)
			this._switchTab(iTabIndex);
		else{
			$("[rel="+openTabid+"]").click();
	...
	loadUrl:function(url){
		var $panel=this._getPanels().eq(this._currentIndex);
		this._getTabs().eq(this._currentIndex).attr("url",url);
		$panel.loadUrl(url, {}, function(){navTab._loadUrlCallback($panel);});
		
	},
	getTitle:function(){
		return this._getTabs().eq(this._currentIndex).find("span").text();
	},
