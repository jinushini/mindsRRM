...
function validInit(){
	//为INPUT框添加事件
	$("[type=text].required,textarea.required,select.required,[type=file].required",".required-validate").blur(function(){
		if($(this).val()!=""){ 
				$(this).removeClass("validError");
				$(this).parent().prev().removeClass("validError");
				$(this).attr("validError","false");
			}else{ 
				$(this).addClass("validError");
				$(this).parent().prev().addClass("validError");
				$(this).attr("validError","true");
			}
	})
	
	
	$("span.required",".required-validate").blur(function(){
	...
	$("span.required",".required-validate").click(function(){
		$(this).removeClass("validError");
		$(this).parent().prev().removeClass("validError");
		$(this).attr("validError","false");
	})
...
function _valid($form){
	validInit();
	var $element=$(".required",$form);
	var errors=0
	
	var i;
	for(i=0;i<$element.length;i++){
		errors+=_validProxy($element.eq(i));
	}
	if(errors!=0){
		alertMsg.error("您有<font color='red'>"+errors+"</font>处数据填写不完整，请填写完整后提交。");
		return false;
	}else
		return true;
}
 ...
 * @return {TypeName} 
 */
function _validProxy($element){
	$element.blur();
	if($element.attr("validError")=="true"){
		return 1;
	}else{
		return 0;
	}
...
function validForm($form){
	if(!$form){
		var $form=navTab.getCurrentPanel().find("FORM");
	}
	return _valid($form);
}
 ...
 * @param {Object} callback
 * @param {String} confirmMsg 提示确认信息
 * @param {boolean} hasValid
 */
function submitForm(callback, confirmMsg,hasValid){
	var $form=navTab.getPanel(navTab.getCurrentTabId()).find("FORM");
	
	if(!hasValid&&!hasValid){
		if (!_valid($form)) {return false;}
	}
 ...
 * 通过JS提交dialog表单
 * @param {Object} callback
 * @param {Object} confirmMsg
 * @return {TypeName} 
 */
function submitDialogForm(callback, confirmMsg){
	var $form=$.pdialog.getCurrent().find("FORM");
	if (!_valid($form)) {return false;}
	var _submitFn = function(){
		$form.find(':focus').blur();
 ...
 * @return {TypeName} 
 */
function submitDialogFileForm(callback, confirmMsg){
	var $form=$.pdialog.getCurrent().find("FORM");
	//anlb修改
	if(!_valid($form)) {return false;}
	if ($iframe.size() == 0) {
 ...
 * @param {Object} callback
 */
function submitFileForm(callback){
	var $form=navTab.getPanel(navTab.getCurrentTabId()).find("FORM"), $iframe = $("#callbackframe");
	//anlb修改
	if(!_valid($form)) {return false;}
	if ($iframe.size() == 0) {
		$iframe = $("<iframe id='callbackframe' name='callbackframe' src='about:blank' style='display:none'></iframe>").appendTo("body");
...
function navTabAjaxDone(json){
	...
	if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok){
		if (json.navTabId){ //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
			//anlb修改
			if(json.navTabId=="currentNavTab")
				navTab.reloadFlag(navTab.getCurrentTabId());
			else
