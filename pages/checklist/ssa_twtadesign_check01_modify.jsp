...
<script type="text/javascript">
	...
	jn(function(){
		...
		if(jn("#taskName").val()=="产保"){//因为借用的产保流程，所以识别节点为产保
			thisClass = "approvegy";
		}
		...
		for(var i=0;i<jn(".tbody tr").length;i++){
			...
			if(jn("#taskName").val() == "产保"){//因为借用的产保流程，所以识别节点为产保
				$thisRow.find(".approvegy input").show();
				$thisRow.find(".approvegy input").attr("name","checkResultList["+ i +"].fourthCheck");
			}
			if(jn("#taskName").val() == "审核"){
...
<div class="pageContent">
  <form method="post" enctype="multipart/form-data" action='../checklist/checklistAppr!chanbaoCheckModifySave.action' class="pageForm required-validate" onsubmit="return iframeCallback(this, navTabAjaxDone);">
			...
			<table width="100%" class="list_table list" style="margin:5px auto;table-layout:fixed;word-break:break-all;word-warp:break-word;">
				<thead>
					<tr align="center">
						<th width="11%">项目大类</th>
						<th width="30%">检查项目</th>
						<th width="15%" class="modify">设计师自查</th>
						<th width="15%" class="proof">校对复核</th>
						<th width="15%" class="audit">审核人员意见</th>
