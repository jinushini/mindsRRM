					...
					<s:iterator value="checkItemList" status="st">
						<tr>
							...
							<td style="padding:5px 15px;" align="left" class="firstCheck">
								<s:if test="(#st.index+1==6)||(#st.index+1==7)||(#st.index+1==8)||(#st.index+1==12)||(#st.index+1==24)">
									<input type="radio" value="是" checked/>是
									<input type="radio" value="否" />否
									<input type="radio" value="不适用" />不适用
								</s:if>
								<s:elseif test="#st.index+1==14">
									<input type="radio" value="无特殊工艺" checked/>无特殊工艺<br/>
									<input type="radio" value="有特殊工艺，但可进行投产" />有特殊工艺，但可进行投产<br/>
									<input type="radio" value="对产品结构进行修改 " />对产品结构进行修改 
								</s:elseif>
								<s:elseif test="#st.index+1==21">
									<input type="radio" value="是，但产品稳定可靠 " checked/>是，但产品稳定可靠 <br/>
									<input type="radio" value="否" />否<br/>
									<input type="radio" value="结构修改" />结构修改
								</s:elseif>
								<s:else>
									<input type="radio" value="是" checked/>是
									<input type="radio" value="否" />否
							...
							<td align="center" class="secondCheck">
								<s:if test="#st.index+1==6">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="设计报告体现产品仿真网格数及仿真收敛精度"/>
								</s:if>
								<s:elseif test="#st.index+1==13">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="①HFSS 全场仿真结果  &#13; ②基于Y 参数的协同仿真结果，协同仿真时，只对频率进行调谐，并给出最终调谐电容 ③MuWave 仿真结果，并说明仿真参数设置（针对波导滤波器滤波器）"/>
								</s:elseif>
								<s:elseif test="#st.index+1==14">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="（尽量避免电火花工艺等）"/>
								</s:elseif>
								<s:elseif test="#st.index+1==17">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="负耦合杆的支撑介质需要通过盖板上安装螺钉来紧固，即，需要在盖板上预留螺纹孔，用螺钉来顶死介质"/>
								</s:elseif>
								<s:elseif test="#st.index+1==23">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="在满足强度设计的同时尽量采用柔性支架设"/>
								</s:elseif>
								<s:elseif test="#st.index+1==24">
									<input type="text" style="border:0;width:91%;font-size:15px;margin:4px 0 4px 4px;padding:5px;" readonly="readonly" value="散热安装面的设计 ,散热支架的设计"/>
								</s:elseif>
								<s:else>
