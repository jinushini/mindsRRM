			...
			for(var j=0;j<$thisRow.find("div").length;j++){
				...
				if(jn("#taskName").val() == "质量"){
					if(i==0||i==1||i==2||i==3||i==4){
						...
						if(j==2){
							$thisDiv.parent().find("input").attr("name","checkResultList["+ i +"].thirdCheck");
						}
					}else{
						$thisDiv.parent().text("").append($thisDiv);
					}
				}else if(jn("#taskName").val() == "工艺"){
					if(i==12||i==13||i==14||i==15||i==16||i==17||i==18||i==19||i==20||i==21||i==22||i==23||i==24||i==25||i==26||i==27||i==28||i==30||i==31||i==32||i==33||i==34||i==35||i==36||i==37||i==38||i==40||i==43||i==44){
						if(j==0){
							$thisDiv.parent().text("").append($thisDiv);
						}
						if(j==1){
				...
				}else if(jn("#taskName").val() == "产保"){
					if(i==5||i==6||i==7||i==8||i==9||i==10||i==11||i==30||i==39||i==41||i==42||i==45){
						if(j==0){
							$thisDiv.parent().text("").append($thisDiv);
						}
						if(j==1){
		...
		<fieldset>
			<table width="98%" class="edit_table" style="margin:5px auto"> 
				...
				<tr>
					...
					产品等级：
						<td style="padding-left:5%;"><s:property value="checklist.productGrade" /> </td>
					</td>
				</tr>
			</table>
		</fieldset>
				...
				<tbody class="tbody">
					<s:iterator value="checkResultList" status="st" >
						<tr target="id" rel="<s:property value="id"/>">
							...
							<td style="padding:5px 15px;">
								<div style="padding:5px;font-size:15px;white-space:normal;word-break:break-all;word-wrap:break-word; "><s:property value='thirdCheck'/></div>
								<input type="text" style="border:0;width:96%;font-size:15px;margin:4px 0 4px 4px;padding:5px;">
							</td>
						</tr>
					</s:iterator>
