...
<script type="text/javascript">
	...
	jn(".funClone").click(function(){
		var id = jn(".mcmList .selected").attr("rel");
		if(id == null){
		...
		$.ajax({
			...
			success : function(data, statusText) {
				...
				if(info != null){
					...
					options.width = 550;
					options.height = 500;
					var url = "../designInfo/Mcm_showMcm_mcm_mcm-detail-clone-input.action?num=" + id;
			...
			<table class="edit_table" width="95%" style="table-layout:fixed;margin:0px;">
				<tr>
					...
					<td style="width: 20%;padding:0px 0px 1px;border:hidden;border:hidden;">
						<select name="paramMap['m.simplifyCode eq']" id="type" style="width:80%;">
							<option id="empty"></option>
							<option <s:if test="paramMap['m.simplifyCode eq']=='MCM-R'">selected</s:if>>MCM-R</option>
							<option <s:if test="paramMap['m.simplifyCode eq']=='MCM-T'">selected</s:if>>MCM-T</option>
							<option <s:if test="paramMap['m.simplifyCode eq']=='MCM-TR'">selected</s:if>>MCM-TR</option>
							<option <s:if test="paramMap['m.simplifyCode eq']=='MCM-FS'">selected</s:if>>MCM-FS</option>
						</select>
					</td>
...
<div class="pageContent" >
	<div class="panelBar">
		<ul class="toolBar">
			<li><a href="../designInfo/Mcm_inputNewMcm_mcm_new-mcm-input.action" class="add" target="navTab"><span>添加新产品</span></a></li>
			<li><a href="../designInfo/Mcm_inputUpgradeMcm_mcm_upgrade-mcm-input.action?num={id}" class="add" target="navTab"><span>升级</span></a></li>
			<li><a title="MCM基本信息" href="../designInfo/Mcm_showMcm_mcm_mcm-name-show.action?num={id}" class="icon" target="dialog" height="550" width="500"><span>产品管理</span></a></li>
			<li><a title="确定要废弃吗" href="../designInfo/Mcm_abandon_mcm.action?num={id}" class="delete" target="ajaxTodo"><span>废弃</span></a></li>
			<li><a class="icon funClone" ><span>性能复制</span></a></li>
			<li><a title="确定将该产品加入优选目录吗" href="../designInfo/Mcm_addCatalog_mcm.action?num={id}" class="edit" target="ajaxTodo"><span>加入目录</span></a></li>
			<%-- <li><a href="../designInfo/Mcm_exchange.action" class="edit" target="ajaxTodo"><span>历史数据处理</span></a></li> --%>
		</ul>
	</div>
	<div layoutH="90">
		<table class="list_table list mcmList" width="100%" layoutH="145" style="table-layout:fixed;">
			<thead>
				<tr>
					<th width="35">序号</th>
