...
<script type="text/javascript">
	...
	jn(".funClone").click(function(){
		var id = jn("table .selected").attr("rel");
		if(id == null){
		...
		$.ajax({
			...
			success : function(data, statusText) {
				...
				if(info != null){
					...
					$.pdialog.open(url, "new_msg", "性能复制", options);
				}
			}
		});
	})
			...
			<table class="edit_table" width="95%" style="table-layout:fixed;margin:0px;">
				<tr>
					...
					<td style="width: 20%;padding:0px 0px 1px;border:hidden;border:hidden;">
						<select name="paramMap['m.simplifyName like']" id="type" style="width:80%;">
							<option id="empty"></option>
							<option <s:if test="paramMap['m.simplifyName like']=='通用放大器'">selected</s:if>>通用放大器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='低噪声放大器'">selected</s:if>>低噪声放大器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='功率放大器'">selected</s:if>>功率放大器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='增益可调放大器'">selected</s:if>>增益可调放大器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='移相衰减器'">selected</s:if>>移相衰减器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='倍频器'">selected</s:if>>倍频器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='混频器'">selected</s:if>>混频器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='自动增益控制放大器'">selected</s:if>>自动增益控制放大器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='线性化器'">selected</s:if>>线性化器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='开关'">selected</s:if>>开关</option>
							<option <s:if test="paramMap['m.simplifyName like']=='调制器'">selected</s:if>>调制器</option>
							<option <s:if test="paramMap['m.simplifyName like']=='噪声源'">selected</s:if>>噪声源</option>
							<option <s:if test="paramMap['m.simplifyName like']=='功分合成器'">selected</s:if>>功分合成器</option>
						</select>
					</td>
