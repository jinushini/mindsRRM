...
<div class="pageContent">
  <form action="../ssaps/ssaps!modifySsapsNode.action" id="searchFrm" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);" >
  		<div class="pageFormContent" layoutH="56">
  		<input type="hidden" name="ssapsNodeId" value="<s:property value="ssapsNode.id"/>">
		<br/>
	<fieldset>
		<legend>节点编辑</legend>
			<table class="list_table edit_table" width="98%" style="table-layout:fixed;" >
				...
				<tbody>
					<tr>
						<td align="center"><s:property value="ssapsNode.nodesName"/></td>
						<td align="center"><input type="text" name="ssapsNode.scheduleDate.planBeginDate" value="<s:date name="ssapsNode.scheduleDate.planBeginDate" format="yyyy-MM-dd"/>" readonly="readonly" class="startDate" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate: new Date(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).parent().parent().find('.endDate').focus();}})"/></td>
						<td align="center"><input type="text" name="ssapsNode.scheduleDate.planEndDate" value="<s:date name="ssapsNode.scheduleDate.planEndDate" format="yyyy-MM-dd"/>" readonly="readonly" class="endDate" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:$(this).parent().parent().find('.startDate').val(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).blur();}})"/></td>
					</tr>
				</tbody>
			</table>
		</fieldset>
