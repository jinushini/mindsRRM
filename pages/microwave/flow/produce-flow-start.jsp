...
<div class="pageContent">
  <form id="formdata" method="post" enctype="multipart/form-data" action='../microwave/flow_ProductProduceProAppr_saveAndSartProject.action' class="pageForm required-validate" onsubmit="return iframeCallback(this, navTabAjaxDone);">
	<input type="hidden" id="id" name="productProduceProId" value='<s:property value="productProducePro.id"/>'/>
	<input type="hidden" id="processInstId" name="processInstId" value='<s:property value="productProducePro.flowRun.processInstId"/>'/>
		...
		<fieldset>
			...
			<table bproduct="0" width="98%" class="edit_table" align="center">
				<tr>
					...
					<td hidden="hidden"><input type="hidden" name="id" value="<s:property value='productProducePro.productProduce.id'/>" class="required" readonly="readonly"></td>
					<td>
						<input name="code" style="width:90%;" type="text" class="required" readonly="readonly" value="<s:property value='productProducePro.productProduce.productBasic.code'/>" suggestFields="code,name,id" suggestUrl="" lookupGroup="productProduce" lookupPk="code" onclick="$(this).next().attr('href','../microwave/product_ProductProduce_showList_produce-list-dialog.action');$(this).next().click()"/>
						<a title="选择产品" class="model" href="###" lookupGroup="" lookupPk="code" target="dialog" width="1080" height="600" ></a>
					</td>
					<td align="center">产品名称：</td>
					<td>
						<input name="name" style="width:90%;" type="text"  class="required" readonly="readonly" value="<s:property value='productProducePro.productProduce.productBasic.name'/>" suggestFields="code,name,id" suggestUrl="" lookupGroup="productProduce" lookupPk="code" onclick="$(this).next().attr('href','../microwave/product_ProductProduce_showList_produce-list-dialog.action');$(this).next().click()"/>
						<a title="选择产品" class="model" href="###" lookupGroup="" lookupPk="code" target="dialog" width="1080" height="600"></a>
					</td>
				</tr>
			...
			<legend>配置参数--<s:property value="productProducePro.flowRun.flowName"/></legend>
			<table bproduct="0" width="98%" class="edit_table fluwRunConfig" align="center">
				...
				<s:iterator value="productProducePro.flowRun.nodes" var="a">
					...
					<s:iterator value="#attr.a.flowRunDatas" var="b">
						<tr class="customflow" level="2">
							...
							<td align="center"><input class="dates" name="flowRunNodeVoList[#index].scheduleDate.manHours" value='<s:property value="scheduleDate.manHours!=null?scheduleDate.manHours:scheduleDate.standardHours"/>' onblur="$(this).parent().parent().find('.endDate').val(convertToDate(Date.parse($(this).parent().parent().find('.startDate').val())+(($(this).val()-1)*1000*60*60*24)));" style="width:98%;border:0;font-size:15px;text-align:center;" ></td>
							<td align="center">
								<input type="hidden" name="flowRunNodeVoList[#index].handler.id" value=''/>
								<input type="text" name="flowRunNodeVoList[#index].handler.name" value='' readonly="readonly" onclick="$(this).next().click()" style="width:98%;border:0;font-size:15px;text-align:center;" />
