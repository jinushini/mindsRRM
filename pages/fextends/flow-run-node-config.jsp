...
<script type="text/javascript">
	...
	$(".addBtn").click(function(){
		...
		jd("[name='newDataList[#index].deployementId']").unbind("change").bind("change", function(){
			$thisTr = jd(this).parent().parent();
			$flowName = $thisTr.find("[name='newDataList[#index].flowName']");
			...
			$.ajax({
				...
				success : function(data, statusText) {
					...
					for(var i=0;i<data.flowModel.flowModelNodes.length;i++){
						...
						coinSet = coinSet + "<div style='font-size:15px;width:100%;'>"
											+ "<input type='hidden' name='newDataList[#index].coinList[#seq].parentNodeId' value='" + flowModelNode.nodesId + "' />"
											+ "<input type='hidden' name='newDataList[#index].coinList[#seq].parentNodeName' value='" + flowModelNode.nodesName + "' />"
											+ "<label style='width:60%;text-align:right;float:left;'>" + flowModelNode.nodesName + ":</label><input type='text' name='newDataList[#index].coinList[#seq].quantity' value='" + flowModelNode.nodeCoin + "' style='padding:1px 1px; border:1px solid #ccc; height:15px; width:30%;float:right;'/><br/>"
				                          + "</div>";
					}
					var $coinSet = $(coinSet);
					$coinSet.initUI();
		...
		alertMsg.confirm("确认删除", {
			okCall: function(){
				$.ajax({
					...
					type : 'post',
					dataType : "json",
					url : "../fextends/FlowRun_deleteFlowRunConfig.action",
					data:{
						id:subFlowId
					}, 
					success : function(data, statusText) {
...
<div class="pageContent">
	<form action="../fextends/FlowRun_bindFlowRunConfig.action" id="searchFrm" method="post" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone);" >
  		<div class="pageFormContent" layoutH="50">
	  		<input type="hidden" name=id value="<s:property value="flowRun.id"/>">
	  		<input type="hidden" name="mark" value="<s:property value="mark"/>">
	  		<input type="hidden" name="parentFlowRunId" value="<s:property value="flowRun.parentFlow.id"/>">
			<br/>
			<fieldset>
				<legend>
					<s:property value="flowRun.flowDefName"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<tbody class="tbody">
						<tr class="modelTr" style="display:none;">
							<td align="center" class="num">
								...
								<input type="hidden" name="newDataList[#index].num" />
							</td>
							<td>
								<select name="newDataList[#index].deployementId" style="width:90%;line-hight:30px;">
							...
							<td align="center"><input name="newDataList[#index].flowName" style="width:90%;border:0;font-size:15px;text-align:center;"></td>
							<td align="center" hidden="hidden">
								<select name="newDataList[#index].autoStart" style="width:90%;text-align:center;">
									<option value="是">是</option>
									<option value="否">否</option>							
								</select>
							</td>
							<td align="center">
								<input name="newDataList[#index].responsibleId" type="hidden">
								<input name="newDataList[#index].responsibleName" type="text" onclick="$(this).next().click()" readonly="readonly" style="width:98%;border:0;font-size:15px;text-align:center;">
								<a height="410" width="715" href="../system/user!findMUser.action" target="dialog" rel="selectUser" resizable="false" maxable="false">
								</a>
							</td>
							<td align="center"><input class="startDate" name="newDataList[#index].scheduleDate.planBeginDate" readonly="readonly" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate: new Date(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).parent().parent().find('.dates').val(Math.abs(Date.parse($(this).val())-Date.parse($(this).parent().parent().find('.endDate').val()))/1000/60/60/24+1);$(this).parent().parent().find('.endDate').focus();} })" style="width:98%;border:0;font-size:15px;text-align:center;"/></td>
							<td align="center"><input class="endDate" name="newDataList[#index].scheduleDate.planEndDate" readonly="readonly" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:$(this).parent().parent().find('.startDate').val(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).parent().parent().find('.dates').val(Math.abs(Date.parse($(this).parent().parent().find('.startDate').val())-Date.parse($(this).val()))/1000/60/60/24+1);$(this).blur();} }) " style="width:98%;border:0;font-size:15px;text-align:center;"/></td>
							<td align="center"><input class="dates" name="newDataList[#index].scheduleDate.manHours" value='<s:property value="scheduleDate.manHours"/>' style="width:98%;border:0;font-size:15px;text-align:center;" ></td>
							<td align="center">					
						...
						<s:iterator value="list" status="st" var="a">
							<tr class="modifyTr">
								<td align="center" class="num">
									...
									<input type="hidden" name="oldDataList[#index].num" />
								</td>
									
								<td align="center" style="display: none;"><input type="text" name="oldDataList[#index].subFlowId" value="<s:property value="id"/>"></td>
								<td align="center"><s:property value="flowDefName"/></td>
								<td align="center"><input name="oldDataList[#index].flowName" value="<s:property value='flowName'/>" style="width:99%;border:0;font-size:15px;text-align:center;"></td>
								<td align="center" hidden="hidden">
									<select name="oldDataList[#index].autoStart" style="width:90%;text-align:center;">
										<option value="是">是</option>						
										<option value="否" <s:if test='autoStart=="否"'>selected</s:if> >否</option>						
									</select>
								</td>
								<td align="center">
									<input name="oldDataList[#index].responsibleId" type="hidden" value="<s:property value="responsibleId"/>">
									<input name="oldDataList[#index].responsibleName" type="text" onclick="$(this).next().click()" readonly="readonly" value="<s:property value="responsibleName"/>" style="width:98%;border:0;text-align:center;">
									<a height="410" width="715" href="../system/user!findMUser.action" target="dialog" rel="selectUser" resizable="false" maxable="false"></a>
								</td>
								<td align="center"><input class="startDate" name="oldDataList[#index].scheduleDate.planBeginDate" value="<s:date name='scheduleDate.planBeginDate' format='yyyy-MM-dd'/>" readonly="readonly" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate: new Date(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).parent().parent().find('.dates').val(Math.abs(Date.parse($(this).val())-Date.parse($(this).parent().parent().find('.endDate').val()))/1000/60/60/24+1);$(this).parent().parent().find('.endDate').focus();} })" style="width:98%;border:0;font-size:15px;text-align:center;"/></td>
								<td align="center"><input class="endDate" name="oldDataList[#index].scheduleDate.planEndDate" value="<s:date name='scheduleDate.planEndDate' format='yyyy-MM-dd'/>" readonly="readonly" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd', minDate:$(this).parent().parent().find('.startDate').val(), doubleCalendar:true, isShowClear:false, onpicked:function(){$(this).parent().parent().find('.dates').val(Math.abs(Date.parse($(this).parent().parent().find('.startDate').val())-Date.parse($(this).val()))/1000/60/60/24+1);$(this).blur();} }) " style="width:98%;border:0;font-size:15px;text-align:center;"/></td>
								<td align="center"><input class="dates"  name="oldDataList[#index].scheduleDate.manHours" value='<s:property value="scheduleDate.manHours"/>' style="width:98%;border:0;font-size:15px;text-align:center;"></td>
								...
								<td class="coinSet" style="padding:5px 3px;">
									<s:iterator value="coinSet" >
										<div style="font-size:15px;width:100%">
											<input type="hidden" name="oldDataList[#index].coinList[#seq].parentNodeName" value="<s:property value="parentNodeName" />" />
											<label style="width:60%;text-align:right;float:left;"><s:property value="parentNodeName" />:</label>
											<input type="text" name="oldDataList[#index].coinList[#seq].quantity" value="<s:property value='quantity'/>" style="padding:1px 1px; border:1px solid #ccc; height:15px; width:30%;float:right;"/>	<br/>
										</div>
									</s:iterator>
								</td>
							</tr>
