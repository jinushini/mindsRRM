			...
			<table width="98%" class="list_table list" style="margin:5px auto">
				...
				<tbody class="tbody">
					<s:iterator value="list" status="st" var="a">
						<s:if test="#a.options != 'parent'">
							<tr>
								<td align="center" title="<s:property value="#attr.a.flowDefName"/>" style="padding-left:8px;word-break:keep-all;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" rowspan="<s:property value='#attr.a.nodes.size' />" ><s:property value="flowDefName"/></td>
								<s:iterator value="#attr.a.nodes" var="b" status="stb">
									<s:if test="#stb.index==0">
										<td align="center">
											<input type="hidden" value="<s:property value='#attr.b.id'/>" name="flowModelNodeVoList[#index].id" />
											<s:property value="#attr.b.flowDefName"/>
										</td>
					...
					<s:iterator value="list" status="st" var="a">
						<s:if test="options=='parent'">
							<tr target="id" rel="<s:property value="deploymentId"/>" class="tr">
								...
								<td>
									<s:iterator value="#attr.a.nodes" var="c" status="stc">
										<s:property value="#attr.c.flowDefName"/>&nbsp;
										<s:if test="!#stc.last">
											＞&nbsp;
										</s:if>
									</s:iterator>
